...
class UsersListScreen(Screen):
    def __init__(self, users, get_data_from_user, user_id):
        ...
        for user in self.users:
            ...
            # item_icon = IconRightWidget(icon='bag-personal', on_release=lambda x: UsersListController.on_item_pressed(self, user["id"], user["name"], get_data_from_user, user_id))            
            # item_icon.bind(on_release=self.on_item_pressed( user["id"], user["name"], get_data_from_user, user_id))
            # self.item.add_widget(item_icon)
        ...
        if(self.users == []):
            self.layout.add_widget(MDLabel(text='Você é o primeiro a usar :))', halign='center', font_style='Subtitle1'))
    ...
    #         # def go_to_screen(self):
                ...
                # App.get_running_app().root.current = 'inventory_screen'
            # print(App.get_running_app().root)
            # Clock.schedule_once(go_to_screen(self))